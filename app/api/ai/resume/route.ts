...
6. skills 按目标岗位和项目素材动态归纳为 3-5 个技能类别，类别名由素材决定，不要套固定模板；可以是“前端工程”“接口与集成”“内容管理”“部署运维”“AI 应用”等，但必须贴合实际项目。每个技能分组只能返回 content 字段，content 必须是 Markdown 列表，包含 1-3 条以 "- " 开头的内容；不要返回单行关键词，不要返回 items 字段。只写能从项目素材中支撑的内容，不写“精通/熟练/了解”、百分比、评分或进度条。
