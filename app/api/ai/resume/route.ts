...
6. skills 按主流技术简历写法归纳为 3-5 个技能类别；类别名短而具体，如“前端框架与语言”“跨端与客户端”“后端与接口”“工程化与部署”“AI 与数据”。每个技能分组只能返回 content 字段，content 是 Markdown 文本，可以是“React、Next.js、TypeScript”这种关键词行，也可以是 2-3 条 "- " 列表；不要返回 items 字段。只写能从项目素材中支撑的内容，不写“精通/熟练/了解”、百分比、评分或进度条。
...
function normalizeSkillContent(skill: AiSkillResponse) {
  ...
  return normalizeStringList(skill.items).join("、");
}
